...
flux_nontriv = np.load(data_path + 'fluxes/' + "PSI2.npz").items()[1][1]
...
skl = skill(flux_nontriv_recon,flux_nontriv)
R_sq = R_squared(flux_nontriv_recon,flux_nontriv)
time_series(flux_nontriv_recon)
print('Eofs:')
print(skl,R_sq)
...
guessflux = np.reshape(np.mean(np.reshape((np.reshape(PSI2_recon,(-1,64,64))*V1_test),(-1,4096)),1),(-1,1))
flux_nontriv_recon = linearly_regressed(guessflux,flux_nontriv)
skl = skill(flux_nontriv_recon,flux_nontriv)
R_sq = R_squared(flux_nontriv_recon,flux_nontriv)
time_series(flux_nontriv_recon)
