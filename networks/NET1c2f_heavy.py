...
def neuralnetwork(x):
    ...
    with tf.name_scope('conv1'):
        W_conv1 = weight_variable([5, 5, 1, 22])
    ...
    with tf.name_scope('fc1'):
        W_fc1 = weight_variable([8 * 8 * 22, 21])
        B_fc1 = bias_variable([21])
        y_pool3_flat = tf.reshape(y_pool3, [-1, 8*8*22])
