...
def neuralnetwork(x):
    ...
    with tf.name_scope('conv4'):
        W_conv4 = weight_variable([4, 4, 32, 64])
    ...
    with tf.name_scope('pool4'):
        y_pool4 = max_pool_2x2(y_conv5)
        
    with tf.name_scope('conv5'):
        W_conv6 = weight_variable([3, 3, 128, 210])
        B_conv6 = bias_variable([210])
        y_conv6 = tf.nn.relu(conv2d(y_pool4, W_conv6) + B_conv6)  
        
    with tf.name_scope('fc1'):
        W_fc1 = weight_variable([4 * 4 * 210, 200])
        B_fc1 = bias_variable([200])
        y_conv6_flat = tf.reshape(y_conv6, [-1, 4*4*210])
        y_fc1 = tf.nn.relu(tf.matmul(y_conv6_flat, W_fc1) + B_fc1)
