...
The folder other contains various bits of code including and folder called old_code which has code use pre-15th Aug when we change the data from data32_1000 (now deleted) to data256_4000 along with how the data is manipulated.
...
Networks contains network architecture codes

This code works best in an interactive python3 environment (we used Anaconda Spyder). NOTE: if the code uses LATEX text rendering (has something like: from matplotlib import rc, plt.rc('text',usetex=True) at the top) then this is almost always best run from the terminal window. Reset everything after use.


TO USE:
To use (i.e. train a neural network on QG data) simply open QGmain and follow the instructions on there. This should be the only program you need to open (with the exception of, perhaps, the network architecture code).
